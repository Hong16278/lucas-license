...
func (s *LicenseService) Activate(ctx context.Context, in ActivateInput) (*ActivateResult, error) {
	...
	// Prepaid month/year cards do not lose time while waiting in the
	// auto-delivery platform. Their term starts exactly once, here, after the
	// key and lifecycle checks pass on the first activation attempt.
	if lic.Plan != nil && lic.Plan.DurationDays > 0 {
		if err := s.store.StartLicenseTermIfNeeded(ctx, lic, lic.Plan.DurationDays); err != nil {
			return nil, apperr.Internal(err)
		}
	}
	...
	if existing, err := s.store.FindActivation(ctx, lic.ID, in.Identifier); err == nil {
		...
		return &ActivateResult{
			...
			ProductID: lic.ProductID, PlanID: lic.PlanID, PlanName: planName(lic), ValidUntil: lic.ValidUntil,
			Token: token, Meta: responseMeta(),
		}, nil
	}
...
func planName(lic *model.License) string {
	if lic.Plan == nil {
		return ""
	}
	return lic.Plan.Name
}
...
func (s *LicenseService) signToken(lic *model.License, identifier string) (string, error) {
	...
	if lic.Plan != nil && lic.Plan.LicenseType == "perpetual" {
		// Perpetual desktop cards are intentionally usable without future
		// server contact. Revocation cannot reach a machine that stays offline.
		expiresAt = 0
	} else if lic.Plan != nil && lic.Plan.DurationDays > 0 && lic.ValidUntil != nil {
		// A prepaid term is fully offline after activation and expires together
		// with the card instead of requiring periodic heartbeat support.
		expiresAt = lic.ValidUntil.Unix()
	} else if lic.ValidUntil != nil && lic.ValidUntil.Unix() < expiresAt {
		expiresAt = lic.ValidUntil.Unix()
	}
