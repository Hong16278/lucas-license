...
// CreateLicenseBatch atomically creates the batch and all of its unassigned
// card licenses. A partial batch is never visible to the auto-delivery flow.
func (s *Store) CreateLicenseBatch(ctx context.Context, batch *model.LicenseBatch, licenses []*model.License, plan *model.Plan) error {
	if batch.ID == "" {
		batch.ID = newID()
	}
	...
	for _, lic := range licenses {
		lic.BatchID = batch.ID
		if err := s.prepareLicenseForInsert(lic); err != nil {
			return err
		}
		...
		if plan != nil && (plan.LicenseType == "subscription" || plan.LicenseType == "trial") {
			sub := &model.Subscription{
				ID: newID(), LicenseID: lic.ID, PlanID: plan.ID, Status: lic.Status,
			}
...
func (s *Store) ListLicenseBatches(ctx context.Context, productID string) ([]*model.LicenseBatch, error) {
	var out []*model.LicenseBatch
	q := s.DB.NewSelect().Model(&out).
		Relation("Product").Relation("Plan").
		OrderExpr("license_batch.created_at DESC")
	if productID != "" {
		q = q.Where("license_batch.product_id = ?", productID)
	}
	return out, q.Scan(ctx)
}

func (s *Store) FindLicenseBatchByID(ctx context.Context, id string) (*model.LicenseBatch, error) {
	b := new(model.LicenseBatch)
	return b, s.DB.NewSelect().Model(b).
		Relation("Product").Relation("Plan").
		Where("license_batch.id = ?", id).Scan(ctx)
}

func (s *Store) ListLicensesByBatch(ctx context.Context, batchID string) ([]*model.License, error) {
	var out []*model.License
	err := s.DB.NewSelect().Model(&out).
		Relation("Product").Relation("Plan").
		Where("license.batch_id = ?", batchID).
		OrderExpr("license.created_at ASC").Scan(ctx)
	return out, err
}
