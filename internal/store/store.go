...
// StartLicenseTermIfNeeded starts a prepaid card's term on first activation.
// The guarded update makes concurrent activation requests share one start time.
func (s *Store) StartLicenseTermIfNeeded(ctx context.Context, l *model.License, durationDays int) error {
	if durationDays <= 0 || l.ValidUntil != nil {
		return nil
	}

	now := time.Now()
	until := now.Add(time.Duration(durationDays) * 24 * time.Hour)
	if _, err := s.DB.NewUpdate().Model((*model.License)(nil)).
		Set("valid_from = ?", now).
		Set("valid_until = ?", until).
		Set("updated_at = ?", now).
		Where("id = ?", l.ID).
		Where("valid_until IS NULL").Exec(ctx); err != nil {
		return err
	}
	...
	if err := s.DB.NewSelect().Model((*model.License)(nil)).
		Column("valid_from", "valid_until").
		Where("id = ?", l.ID).
		Scan(ctx, &term); err != nil {
		return err
	}
	l.ValidFrom = term.ValidFrom
	l.ValidUntil = term.ValidUntil
	return nil
}
