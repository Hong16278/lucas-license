...
type Plan struct {
	...
	// DurationDays starts on the first successful activation. Zero means
	// perpetual or lifecycle managed by an external billing provider.
	DurationDays int `bun:",notnull" json:"duration_days"`
...
type License struct {
	...
	ProductID  string `bun:",notnull" json:"product_id"`
	PlanID     string `bun:",notnull" json:"plan_id"`
	BatchID    string `bun:",nullzero" json:"batch_id,omitempty"`
...
// LicenseBatch groups unassigned card keys generated for an external
// auto-delivery platform. Sales state intentionally remains outside Keygate;
// this platform only tracks whether a key has been activated.
type LicenseBatch struct {
	bun.BaseModel `bun:"table:license_batches"`

	ID        string    `bun:",pk" json:"id"`
	ProductID string    `bun:",notnull" json:"product_id"`
	PlanID    string    `bun:",notnull" json:"plan_id"`
	Name      string    `bun:",notnull" json:"name"`
	Channel   string    `bun:",notnull" json:"channel"`
