...
func (h *AdminHandler) UpdatePlan(c *gin.Context) {
	...
	if req.DurationDays != nil {
		check.DurationDays = *req.DurationDays
	}
...
func validatePlanNumericBounds(b planBounds) error {
	...
	case b.DurationDays < 0:
		return fmt.Errorf("duration_days cannot be negative")
	case b.DurationDays > 36500:
		return fmt.Errorf("duration_days cannot exceed 36500")
...
// CreateLicenseBatch creates unassigned prepaid cards for export to an
// external auto-delivery platform. Customer/order data deliberately stays out
// of Keygate, so these licenses have an empty email until activated.
func (h *AdminHandler) CreateLicenseBatch(c *gin.Context) {
	var req struct {
		ProductID string `json:"product_id" binding:"required"`
		PlanID    string `json:"plan_id" binding:"required"`
		Name      string `json:"name" binding:"required"`
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		response.BadRequest(c, "product_id, plan_id, name, and quantity are required")
		return
	}
	req.Name = strings.TrimSpace(req.Name)
	req.Channel = strings.TrimSpace(req.Channel)
	if req.Quantity < 1 || req.Quantity > 5000 {
		response.BadRequest(c, "quantity must be between 1 and 5000")
		return
	}
	if req.Name == "" || len(req.Name) > 100 {
		response.BadRequest(c, "name must be between 1 and 100 characters")
		return
	}
	if len(req.Channel) > 100 {
		response.BadRequest(c, "channel cannot exceed 100 characters")
		return
	}
	...
	if plan.ProductID != req.ProductID {
		response.Err(c, http.StatusBadRequest, "PLAN_PRODUCT_MISMATCH", "plan does not belong to the requested product")
		return
	}
	if !plan.Active {
		response.BadRequest(c, "plan is inactive")
		return
	}
	if !requireKeyProductScope(c, req.ProductID) {
		return
	}
	product, err := h.Store.FindProductByID(c, req.ProductID)
	if err != nil {
		response.NotFound(c, "product not found")
		return
	}

	prefix := licenseKeyPrefix(product.Slug)
	licenses := make([]*model.License, 0, req.Quantity)
	codes := make([]string, 0, req.Quantity)
	for range req.Quantity {
		code := license.GenerateKey(prefix)
		licenses = append(licenses, &model.License{
			...
			LicenseKey: code,
			Status:     model.StatusActive,
			Notes:      "batch:" + req.Name,
		})
		codes = append(codes, code)
	}

	batch := &model.LicenseBatch{
		ProductID: req.ProductID,
		PlanID:    req.PlanID,
		Name:      req.Name,
		Channel:   req.Channel,
	...
	if err := h.Store.CreateLicenseBatch(c, batch, licenses, plan); err != nil {
		slog.Error("create license batch", "error", err)
		response.Internal(c)
		return
	}

	h.Store.Audit(c, &model.AuditLog{
		Entity: "license_batch", EntityID: batch.ID, Action: "created",
		ActorType: "admin", ActorID: adminID(c),
		Changes: map[string]any{"name": batch.Name, "quantity": batch.Quantity, "channel": batch.Channel, "plan_id": batch.PlanID},
	})

	response.Created(c, gin.H{"batch": batch, "codes": codes})
}

func licenseKeyPrefix(slug string) string {
...
func (h *AdminHandler) ListLicenseBatches(c *gin.Context) {
	productID := c.Query("product_id")
	if v, ok := c.Get("api_key"); ok {
		if ak, ok := v.(*model.APIKey); ok && ak != nil && ak.ProductID != "" {
			if productID != "" && productID != ak.ProductID {
	...
	batches, err := h.Store.ListLicenseBatches(c, productID)
	if err != nil {
		response.Internal(c)
		return
	}
...
func (h *AdminHandler) ExportLicenseBatch(c *gin.Context) {
	batch, err := h.Store.FindLicenseBatchByID(c, c.Param("id"))
	if err != nil {
		response.NotFound(c, "license batch not found")
		return
	}
	if !requireKeyProductScope(c, batch.ProductID) {
		return
	}
	licenses, err := h.Store.ListLicensesByBatch(c, batch.ID)
	if err != nil {
		response.Internal(c)
		return
	}
	...
	filename := fmt.Sprintf("cards-%s-%s.%s", batch.ID, time.Now().Format("20060102"), format)
	c.Header("Content-Disposition", fmt.Sprintf("attachment; filename=%s", filename))
	if format == "txt" {
		c.Header("Content-Type", "text/plain; charset=utf-8")
		for _, lic := range licenses {
			_, _ = fmt.Fprintln(c.Writer, h.Store.DecryptLicenseKey(lic))
		}
		return
	}
	if format != "csv" {
		response.BadRequest(c, "format must be txt or csv")
		return
	}
	...
	defer w.Flush()
	_ = w.Write([]string{"license_key", "product", "plan", "duration_days", "max_activations", "batch", "channel"})
	for _, lic := range licenses {
		_ = w.Write([]string{
			h.Store.DecryptLicenseKey(lic), batch.Product.Name, batch.Plan.Name,
			strconv.Itoa(batch.Plan.DurationDays), strconv.Itoa(batch.Plan.MaxActivations), batch.Name, batch.Channel,
		})
	}
}
