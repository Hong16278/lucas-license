...
func (h *SetupHandler) Initialize(c *gin.Context) {
	...
	planID := store.NewID()
	// checkout_id became NOT NULL in 20260401_plan_checkout_id. Setup writes
	// through the transaction directly (rather than Store.CreatePlan), so it
	// must populate the field itself or a fresh installation fails with a 500.
	// The plan ID is already globally unique and URL-safe, making it a suitable
	// one-time checkout identifier for the bootstrap plan.
	checkoutID := strings.ToLower(planID)
	...
	if _, err := tx.NewRaw(
		"INSERT INTO plans (id, product_id, name, slug, checkout_id, license_type, max_activations, max_seats, grace_days, active, created_at) VALUES (?, ?, 'Pro', 'pro', ?, 'subscription', ?, ?, 7, true, now())",
		planID, productID, checkoutID, maxActivations, maxSeats,
	).Exec(ctx); err != nil {
		response.Internal(c)
		return
	}
	...
	plan := &model.Plan{
		ID: planID, ProductID: productID, Name: "Pro", Slug: "pro",
		CheckoutID: checkoutID, LicenseType: "subscription",
		MaxActivations: maxActivations, MaxSeats: maxSeats,
	}
