...
export const admin = {
  ...
  createLicenseBatch: (data: {
    product_id: string
    plan_id: string
    name: string
    channel?: string
    quantity: number
  }) => post<{ batch: LicenseBatch; codes: string[] }>("/admin/license-batches", data),
  listLicenseBatches: (productId?: string) => {
    const q = new URLSearchParams()
    if (productId) q.set("product_id", productId)
    return get<{ batches: LicenseBatch[] }>(`/admin/license-batches?${q}`)
  },
  licenseBatchExportUrl: (id: string, format: "txt" | "csv") =>
    `${BASE}/admin/license-batches/${id}/export?format=${format}`,
